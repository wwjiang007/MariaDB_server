...
{
  ut_ad(buf_pool.is_uncompressed(&block) || !is_logged());
...
inline void mtr_t::memset(const buf_block_t &b, const byte *ofs, ulint len,
                          byte val)
{
  ut_ad(len);
  ...
  const uint16_t o= uint16_t(ut_align_offset(ofs, srv_page_size));
...
inline void mtr_t::memset(const buf_block_t &b, const byte *ofs, size_t len,
                          const void *str, size_t size)
{
  ...
  const uint16_t o= uint16_t(ut_align_offset(ofs, srv_page_size));
  byte *l= log_write<MEMSET>(b.page.id(), &b.page, lenlen + size, true, o);
  l= mlog_encode_varint(l, len);
  ::memcpy(l, str, size);
  l+= size;
  m_log.close(l);
                          ...
                          size_t len)
{
  ...
  if (!is_logged())
    return;
  const uint16_t o= uint16_t(ut_align_offset(offset, srv_page_size));
