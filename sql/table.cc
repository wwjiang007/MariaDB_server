...
/**
  @brief
    Copy item_list names to this->original_names
...
bool TABLE_LIST::set_original_names()
{
  st_select_lex *sl = derived->first_select();

  if (unlikely(sl->with_wild))
    return false;
  if (original_names_are_set)
    return false;

  // these elements allocated in LEX::parsed_derived_table
  if (original_names->elements != sl->item_list.elements)
  {
  ...
  List_iterator_fast<Item> item_list_iterator(sl->item_list);
