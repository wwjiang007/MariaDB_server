  ...
  while (unlikely(error=table->file->ha_write_row(table->record[0])))
  {
    ...
    if (can_optimize && key_nr == last_unique_key)
    {
      ...
      if (versioned)
      {
        ...
        if (unlikely(error))
          return on_ha_error_after_modify(error);
      }
...
int Write_record::insert_on_duplicate_update()
{
...
inline
int Write_record::after_insert()
{
  notify_non_trans_table_modified();
  return after_trg_and_copied_inc() || send_data();
}

inline
int Write_record::after_replace()
{
  /*
    If more than one iteration of the replace loop is done, from
...
int Write_record::after_trg_and_copied_inc()
{
  info->copied++;
