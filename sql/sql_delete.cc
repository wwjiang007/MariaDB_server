...
inline
int TABLE::delete_row(bool replace, bool treat_versioned)
{
  int err= 0;
  bool delete_row= !treat_versioned
                && (!versioned(VERS_TIMESTAMP) || !vers_end_field()->is_max());
  if (!delete_row)
  {
    ...
    vers_update_end();
    // Don't preserve rows with negative lifetime: delete them instead.
    // See vers_insert_history_row
  ...
  if (delete_row)
    err= file->ha_delete_row(record[1]);
